...
const COLOURS = {
  ...
  text: "#1e293b",
  muted: "#64748b",
  shadow: "0 10px 26px rgba(11,47,99,0.10)",
};
...
const starterDiary = [
  ...
  },
  {
    ...
    time: "6:30 pm",
    place: "Club Green",
    notes: "Friendly roll-up for all sections.",
  ...
  {
    id: 4,
    title: "Friendly v Woodilee Bowling Club",
    date: "Saturday 16 May 2026",
    time: "2:00 pm",
    place: "Home Match",
    notes: "Rinks to be confirmed by the Match Secretary.",
...
const starterCompetitions = [
  {
    ...
    closes: "31 May 2026",
    starts: "6 June 2026",
    notes: "Open to all full members.",
...
export default function App() {
  ...
  function buildItemFromForm(id) {
    if (adminSection === "diary") {
      return { id, title: form.title, date: form.date, time: form.time, place: form.place, notes: form.notes, fileName: form.fileName, fileUrl: form.fileUrl };
    }
    if (adminSection === "notices") {
      return { id, title: form.title, body: form.body, date: form.date, fileName: form.fileName, fileUrl: form.fileUrl };
    }
    if (adminSection === "comps") {
      return { id, name: form.name, closes: form.closes, starts: form.starts, notes: form.notes, fileName: form.fileName, fileUrl: form.fileUrl };
    }
    if (adminSection === "members") {
      return { id, name: form.name, section: form.section, phone: form.phone, fileName: form.fileName, fileUrl: form.fileUrl };
    }
    if (adminSection === "office") {
      return { id, role: form.role, name: form.name, phone: form.phone, fileName: form.fileName, fileUrl: form.fileUrl };
    }
    if (adminSection === "coaches") {
      return { id, title: form.title, name: form.name, phone: form.phone, fileName: form.fileName, fileUrl: form.fileUrl };
    }
  ...
  return (
    <div style={{ fontFamily: "Arial, Helvetica, sans-serif", background: COLOURS.page, minHeight: "100vh", color: COLOURS.text }}>
      <div style={{ maxWidth: 1320, margin: "0 auto", padding: 18 }}>
        <div style={cardStyle({ overflow: "hidden", marginBottom: 18, borderRadius: 24 })}>
          <div style={{ background: `linear-gradient(135deg, ${COLOURS.navy} 0%, ${COLOURS.blue} 100%)`, color: COLOURS.white, padding: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "90px 1fr auto", gap: 18, alignItems: "center" }}>
              <div style={{ width: 82, height: 82, borderRadius: "50%", background: COLOURS.red, border: `4px solid ${COLOURS.gold}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 900 }}>
                LBC
              </div>
              <div>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "flex-end" }}>
                <div style={{ ...cardStyle({ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.18)", boxShadow: "none", padding: 14, minWidth: 170 }) }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 900, marginTop: 6 }}>{nextEvent?.title || "No event"}</div>
                  <div style={{ fontSize: 14, marginTop: 6 }}>{nextEvent?.date || ""} • {nextEvent?.time || ""}</div>
                </div>
          ...
          <div style={{ background: COLOURS.white, borderTop: `1px solid ${COLOURS.border}`, padding: 14, display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {navItems.map(([key, label]) => (
                <button key={key} onClick={() => setTab(key)} style={{ background: tab === key ? COLOURS.navy : COLOURS.panel, color: tab === key ? COLOURS.white : COLOURS.navy, border: `1px solid ${tab === key ? COLOURS.navy : COLOURS.border}`, borderRadius: 12, padding: "11px 15px", fontSize: 15, fontWeight: 900, cursor: "pointer" }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr 0.9fr", gap: 18 }}>
            <div style={{ display: "grid", gap: 18 }}>
              <div style={cardStyle({ padding: 24 })}>
                ...
                <div style={{ fontSize: 18, lineHeight: 1.6, color: COLOURS.text }}>
                  A clean demonstration app for Lenzie Bowling Club with diary items, notices, competitions, members, office bearers, coaches and documents.
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                ...
                <div style={cardStyle({ padding: 22 })}>
                  ...
                  {[
                    ["Instituted", "1873"],
                    ["Location", "Regent Square, Lenzie"],
                    ["Postcode", "G66 5AE"],
                    ["Green Type", "Grass"],
                    ["Colours", "Navy, red and gold"],
                    ["Club", "The Lenzie Bowling Club Limited"],
                  ].map(([label, value]) => (
        ...
        {tab === "diary" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Diary</div>
            {diaryEvents.map((item) => (
              <div key={item.id} style={{ padding: 18, border: `1px solid ${COLOURS.border}`, borderRadius: 14, marginBottom: 14, background: COLOURS.panel }}>
                <div style={{ fontSize: 22, fontWeight: 900 }}>{item.title}</div>
                <div style={{ fontSize: 15, color: COLOURS.muted, marginTop: 8 }}>{item.date} • {item.time}</div>
                <div style={{ fontSize: 15, marginTop: 8 }}>{item.place}</div>
                <div style={{ fontSize: 15, marginTop: 8 }}>{item.notes}</div>
        ...
        {tab === "members" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
              <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy }}>Members</div>
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search members" style={{ padding: "11px 14px", fontSize: 15, borderRadius: 12, border: `1px solid ${COLOURS.border}`, minWidth: 260 }} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
              {filteredMembers.map((member) => (
                <div key={member.id} style={cardStyle({ padding: 18, background: COLOURS.panel })}>
                  <div style={{ fontSize: 18, fontWeight: 900 }}>{member.name}</div>
                  <div style={{ fontSize: 14, color: COLOURS.red, fontWeight: 800, marginTop: 6 }}>{member.section}</div>
                  <div style={{ fontSize: 15, marginTop: 8 }}>{member.phone}</div>
        ...
        {tab === "notices" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Noticeboard</div>
            {notices.map((item) => (
              <div key={item.id} style={{ padding: 18, border: `1px solid ${COLOURS.border}`, borderRadius: 14, marginBottom: 14, background: COLOURS.panel }}>
                <div style={{ fontSize: 22, fontWeight: 900 }}>{item.title}</div>
                <div style={{ fontSize: 14, color: COLOURS.muted, marginTop: 8 }}>{item.date}</div>
                <div style={{ fontSize: 15, marginTop: 8 }}>{item.body}</div>
                <FileLink name={item.fileName} url={item.fileUrl} />
              </div>
            ))}
        ...
        {tab === "comps" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Competitions</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
              {competitions.map((item) => (
                <div key={item.id} style={cardStyle({ padding: 18, background: COLOURS.panel })}>
                  <div style={{ fontSize: 20, fontWeight: 900 }}>{item.name}</div>
                  <div style={{ fontSize: 15, marginTop: 8 }}>Entry closes: {item.closes}</div>
                  <div style={{ fontSize: 15, marginTop: 6 }}>Starts: {item.starts}</div>
                  <div style={{ fontSize: 15, marginTop: 8 }}>{item.notes}</div>
                  <FileLink name={item.fileName} url={item.fileUrl} />
                </div>
              ))}
        ...
        {tab === "office" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Office Bearers</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
              {officeBearers.map((person) => (
                <div key={person.id} style={cardStyle({ padding: 18, background: COLOURS.panel })}>
                  <div style={{ fontSize: 14, color: COLOURS.red, fontWeight: 800 }}>{person.role}</div>
                  <div style={{ fontSize: 22, fontWeight: 900, marginTop: 8 }}>{person.name}</div>
                  <div style={{ fontSize: 15, marginTop: 8 }}>{person.phone}</div>
        ...
        {tab === "coaches" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Coaches</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
              {coaches.map((coach) => (
                <div key={coach.id} style={cardStyle({ padding: 18, background: COLOURS.panel })}>
                  <div style={{ fontSize: 14, color: COLOURS.red, fontWeight: 800 }}>{coach.title}</div>
                  <div style={{ fontSize: 22, fontWeight: 900, marginTop: 8 }}>{coach.name}</div>
                  <div style={{ fontSize: 15, marginTop: 8 }}>{coach.phone}</div>
        ...
        {tab === "documents" && (
          <div style={cardStyle({ padding: 24 })}>
            <div style={{ fontSize: 30, fontWeight: 900, color: COLOURS.navy, marginBottom: 18 }}>Documents</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
              {documents.map((doc) => (
                <div key={doc.id} style={cardStyle({ padding: 18, background: COLOURS.panel })}>
                  <div style={{ display: "inline-block", padding: "6px 10px", borderRadius: 999, background: "#e7eefb", color: COLOURS.blue, fontSize: 13, fontWeight: 800 }}>{doc.category}</div>
                  <div style={{ fontSize: 22, fontWeight: 900, marginTop: 10 }}>{doc.name}</div>
        ...
        <div style={{ marginTop: 18, background: COLOURS.red, color: COLOURS.white, padding: "16px 22px", textAlign: "center", fontWeight: 900, fontSize: 24, borderRadius: 16 }}>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(1160px, 100%)", maxHeight: "92vh", overflow: "auto", ...cardStyle({ borderRadius: 24 }) }}>
            <div style={{ background: COLOURS.navy, color: COLOURS.white, padding: 20, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <div>
                ...
                <div style={{ fontSize: 15, marginTop: 6, opacity: 0.95 }}>Add, edit or remove items and attach files.</div>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.05fr 1fr", gap: 20 }}>
                  <div style={cardStyle({ padding: 20, background: COLOURS.panel })}>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <input ref={fileRef} type="file" onChange={handleFileChange} style={{ fontSize: 14 }} />
                    </div>
